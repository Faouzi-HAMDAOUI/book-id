  ...
  <v-carousel height="700" hide-delimiters>
    <v-carousel-item v-for="(book, i) in listBooks" :key="i">
      <v-row>
          ...
          <v-sheet color="transparent" class="black--text mr-12 mt-16">
            <v-row>
              <div>
                ...
                <v-chip v-if="book.genre[0]" class="ma-2  mt-1" color="deep-purple darken-1">
                  {{ book.genre[0] }}
                </v-chip>
                <v-chip v-if="book.genre[1]" class="ma-2  mt-1" color="purple darken-1">
                  {{ book.genre[1] }}
                </v-chip>
                <p class="mr-16  mt-12">
                  {{ book.resume }}
        ...
        <v-bottom-navigation class="text-center black--text transparent" height="110">
  ...
  methods: {
    ...
    sendMessage(book) {
      ...
      (this.newMsg = {
        ...
        msg: "Bonjour, je suis intéressé par votre livre, et j'aimerais savoir si je peux l'emprunter ?",
        created: this.date
      }),
        this.getMyMessages();
      setTimeout(() => {
      ...
      this.messageConf = "Votre demande a bien été prise en compte, vous serez contacté prochainement par son propriétaire !"
      this.confMsg = true
      setTimeout(() => {
              return (this.confMsg = false);
           }, 4000);
